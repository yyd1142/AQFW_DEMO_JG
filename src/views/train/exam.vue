<template>
    <div>
        ...
        <mko-header :title="examItem.title" left-icon="icon-back" @handleLeftClick="back"></mko-header>
        <div class="page-wrap exam-wrap">
            <div v-if="startExamShow">
                    ...
                    <ul class="exam-table-view">
                        <li class="exam-table-cell" v-for="(item, index) in subjects">
                            ...
                            <span class="option" v-for="(obj, childIndex) in item.examOption"
                                  @click="exam(index, childIndex)">
                                        <i :class="obj.value ? 'true' : 'false'"></i>
                                        {{obj.label}}
                                    </span>
                        </li>
                    </ul>
                ...
                <div class="footer-bar">
                    <div class="test-btn start-left" :class="timeUpClass">剩余时间：<span id="timer" v-text="time"></span>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            submit() {
                ...
                params['examTopic'] = examTopic;
                Indicator.open({
                    spinnerType: 'fading-circle'
                });
                api.examResult(params).then(result => {
                    if (result.code == 0) {
                        let errors = [];
                        let datas = result.response;
                        submitDatas.forEach((item) => {
            ...
            back() {
                if (this.bInputData && !this.isTimeup) {
                    this.$MKODialog({
