  ...
  <div>
    ...
    <div class="page-wrap qy-list-wrap" v-show="!resError" id="pageWrapper">
      ...
      <div class="header-btn-wrap">
        <div class="header-btn" :class="{active:listType==item.type}" @click="ctrlListType(item.type)" v-for="item in headerBtn">{{item.text}}
        </div>
      </div>
      <!--搜索-->
        ...
        <mt-loadmore ref="loadmore" :bottom-method="loadBottom" @bottom-status-change="handleBottomChange" :auto-fill="autoFill" :bottom-all-loaded="bottomAllLoaded">
          <ul class="list">
            <li class="item" @click="go(item.groupId,item.dwName,item.dwSafeScore)" v-for="(item,n) in dwList">
              <div class="score" :class="{'score-null':!item.dwSafeScore}" :style="{color:calcScoreStyle(item.dwSafeScore)}">
          ...
          <div class="body-wrap">
            <div class="main-wrap">
              ...
              <div class="radio-wrap small-radio-wrap">
                <mt-button class="radio" :class="{'checked':formData.dwSafeScore==item.value}" @click.native="formData.dwSafeScore=item.value" v-for="item in options.score" size="large">
                  <span class="sign has-gutter" v-show="formData.dwSafeScore==item.value"></span>{{item.label}}
                </mt-button>
              </div>
            </div>
            <div class="main-wrap">
              ...
              <div class="radio-wrap">
                <mt-button class="radio" :class="{'checked':formData.dwType==item.dwTypeID}" @click.native="formData.dwType=item.dwTypeID" v-for="item in options.dwType" size="large">
                  <span class="sign has-gutter" v-show="formData.dwType==item.dwTypeID"></span>{{item.dwTypeName}}
                </mt-button>
              </div>
            </div>
...
export default {
  ...
  activated() {
    ...
    if (group_id != this.$store.getters.groupId) {
        ...
        if (res.code == 0) {
          ...
          if (res.response.datas === undefined || res.response.datas.length === 0) {
            ...
            qy_datas[this.listType] = {
              datas: res.response.datas, pageItem: { page: res.response.page, pageCount: res.response.pageCount }
            };
        ...
        api.getQyList(params).then(res => {
          ...
          if (res.code == 0) {
            if (res.response.datas === undefined || res.response.datas.length === 0) {
              ...
              this.noData = false;
              qy_datas[this.listType] = {
                datas: this.dwList, pageItem: { page: res.response.page, pageCount: res.response.pageCount }
              }
            }
          }
          Indicator.close();
