<template>
  <div>
    ...
    <mt-header class="header-wrap" fixed title="消防责任单位">
    ...
    <div class="page-wrap qy-info-wrap"  v-if="!notData">
      <div class="data-wrap">
        <div  v-for="(item, index) in dwList" @click="goDwInfo(index)">
          <mt-cell :title="item.name"  v-if="item.datas.length != 0" is-link>
            <div class="unit-btn">{{item.datas.length}}</div>
          </mt-cell>
        </div>
      </div>
    </div>
